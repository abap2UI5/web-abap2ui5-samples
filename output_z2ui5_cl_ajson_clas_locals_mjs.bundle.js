...
class lcl_utils {
  ...
  static async xstring_to_string_utf8(INPUT) {
    ...
    lv_in_ce.set(new abap.types.Character(18).set('CL_ABAP_CONV_IN_CE'));
    try {
      if (abap.Classes['CL_ABAP_CONV_CODEPAGE'.trimEnd()] === undefined) { if (abap.Classes['CX_SY_DYN_CALL_ILLEGAL_CLASS'.trimEnd()] === undefined) { throw "CX_SY_DYN_CALL_ILLEGAL_CLASS not found"; } else {throw new abap.Classes['CX_SY_DYN_CALL_ILLEGAL_CLASS'.trimEnd()](); } }
      ...
      if (lo_conv.get().if_abap_conv_in$convert === undefined && abap.Classes['CX_SY_DYN_CALL_ILLEGAL_METHOD'.trimEnd()] === undefined) { throw "CX_SY_DYN_CALL_ILLEGAL_METHOD not found"; }
      if (lo_conv.get().if_abap_conv_in$convert === undefined) { throw new abap.Classes['CX_SY_DYN_CALL_ILLEGAL_METHOD'.trimEnd()](); }
      rv_str.set(await lo_conv.get().if_abap_conv_in$convert({source: iv_xstr}));
    } catch (e) {
      if ((abap.Classes['CX_SY_DYN_CALL_ILLEGAL_CLASS'] && e instanceof abap.Classes['CX_SY_DYN_CALL_ILLEGAL_CLASS'])) {
        ...
        lo_conv.set(await abap.Classes[lv_in_ce.get().trimEnd()].create({encoding: new abap.types.Character(5).set('UTF-8')}));
        if (lo_conv.get().convert === undefined && abap.Classes['CX_SY_DYN_CALL_ILLEGAL_METHOD'.trimEnd()] === undefined) { throw "CX_SY_DYN_CALL_ILLEGAL_METHOD not found"; }
        if (lo_conv.get().convert === undefined) { throw new abap.Classes['CX_SY_DYN_CALL_ILLEGAL_METHOD'.trimEnd()](); }
        await lo_conv.get().convert({data: iv_xstr,buffer: rv_str});
      } else {
        throw e;
      }
    }
  ...
  static async any_to_xstring(INPUT) {
    ...
    if (abap.compare.eq(unique1, abap.Classes['CLAS-Z2UI5_CL_AJSON-LIF_KIND'].lif_kind$binary.get().xstring)) {
      ...
      if (abap.compare.ne(lo_table_type.get().table_kind, abap.Classes['CL_ABAP_TABLEDESCR'].tablekind_std)) {
        await abap.Classes['Z2UI5_CX_AJSON_ERROR'].raise({iv_msg: new abap.types.Character(56).set('Unsupported type of input table (must be standard table)')});
      }
      try {
        ...
        if ((abap.Classes['CX_ROOT'] && e instanceof abap.Classes['CX_ROOT'])) {
          await abap.Classes['Z2UI5_CX_AJSON_ERROR'].raise({iv_msg: new abap.types.Character(53).set('Error converting input table (should be string_table)')});
        } else {
          throw e;
        }
      }
    } else {
      await abap.Classes['Z2UI5_CX_AJSON_ERROR'].raise({iv_msg: new abap.types.Character(74).set('Unsupported type of input (must be char, string, string_table, or xstring)')});
    }
    return rv_xstr;
  }
  ...
  async get_node_type(INPUT) {
    ...
    if (abap.compare.ne(abap.builtin.sy.get().subrc, abap.IntegerFactory.get(0))) {
      ...
      if (abap.compare.eq(unique9, abap.Classes['CLAS-Z2UI5_CL_AJSON-LIF_KIND'].lif_kind$table)) {
        ...
        } else if (abap.compare.eq(unique9, new abap.types.Character(1).set(''))) {
    ...
    async any_to_abap(INPUT) {
      ...
      try {
        for await (const unique11 of abap.statements.loop(this.mr_nodes.dereference(),{usingKey: "array_index",where: async (I) => {return abap.compare.eq(I.path, iv_path);},topEquals: {"path": iv_path}})) {
          ...
          } else if (abap.compare.eq(unique12, new abap.types.Character(1).set(''))) {
    ...
    async to_timestamp(INPUT) {
      ...
      try {
        ...
        if (abap.compare.eq(unique15, new abap.types.Character(1).set('-'))) {
          lv_timestamp.set((await abap.Classes['CL_ABAP_TSTMP'].add({tstmp: lv_timestamp, secs: lv_seconds_conv})));
        } else if (abap.compare.eq(unique15, new abap.types.Character(1).set('+'))) {
