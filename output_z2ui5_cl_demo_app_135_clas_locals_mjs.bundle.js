  ...
  static async acquire_lock() {
    ...
    try {
      ...
      if (abap.compare.ne(abap.builtin.sy.get().subrc, abap.IntegerFactory.get(0))) {
        ...
        const unique237 = await (new abap.Classes['Z2UI5_CX_UTIL_ERROR']()).constructor_({val: error_text});
        unique237.EXTRA_CX = {"INTERNAL_FILENAME": "z2ui5_cl_demo_app_135.clas.locals_imp.abap","INTERNAL_LINE": 58};
        throw unique237;
      }
    }
    ...
    static async get_lock_counter() {
      ...
      try {
        ...
        if (abap.compare.ne(abap.builtin.sy.get().subrc, abap.IntegerFactory.get(0))) {
          await abap.statements.message({into: error_text, id: abap.builtin.sy.get().msgid, type: abap.builtin.sy.get().msgty, number: abap.builtin.sy.get().msgno, with: [abap.builtin.sy.get().msgv1,abap.builtin.sy.get().msgv2,abap.builtin.sy.get().msgv3,abap.builtin.sy.get().msgv4]});
          const unique238 = await (new abap.Classes['Z2UI5_CX_UTIL_ERROR']()).constructor_({val: error_text});
          unique238.EXTRA_CX = {"INTERNAL_FILENAME": "z2ui5_cl_demo_app_135.clas.locals_imp.abap","INTERNAL_LINE": 89};
          throw unique238;
        }
        temp1.clear();
