...
class lcl_stream {
  ...
  async append_crc(INPUT) {
    ...
    if (abap.compare.eq(abap.builtin.xstrlen({val: lcl_stream.crc32_map}), abap.IntegerFactory.get(0))) {
      ...
      const unique692 = new abap.types.Integer().set(256).get();
      for (let unique693 = 0; unique693 < unique692; unique693++) {
        abap.builtin.sy.get().index.set(unique693 + 1);
        ...
        const unique694 = abap.IntegerFactory.get(8).get();
        for (let unique695 = 0; unique695 < unique694; unique695++) {
          abap.builtin.sy.get().index.set(unique695 + 1);
    ...
    const unique696 = len.get();
    for (let unique697 = 0; unique697 < unique696; unique697++) {
      abap.builtin.sy.get().index.set(unique697 + 1);
