...
class lcl_stream {
  ...
  async append_crc(INPUT) {
    ...
    if (abap.compare.eq(abap.builtin.xstrlen({val: lcl_stream.crc32_map}), abap.IntegerFactory.get(0))) {
      ...
      const unique672 = new abap.types.Integer().set(256).get();
      for (let unique673 = 0; unique673 < unique672; unique673++) {
        abap.builtin.sy.get().index.set(unique673 + 1);
        ...
        const unique674 = abap.IntegerFactory.get(8).get();
        for (let unique675 = 0; unique675 < unique674; unique675++) {
          abap.builtin.sy.get().index.set(unique675 + 1);
    ...
    const unique676 = len.get();
    for (let unique677 = 0; unique677 < unique676; unique677++) {
      abap.builtin.sy.get().index.set(unique677 + 1);
