...
class lcl_stream {
  ...
  async append_crc(INPUT) {
    ...
    if (abap.compare.eq(abap.builtin.xstrlen({val: lcl_stream.crc32_map}), abap.IntegerFactory.get(0))) {
      ...
      const unique663 = new abap.types.Integer().set(256).get();
      for (let unique664 = 0; unique664 < unique663; unique664++) {
        abap.builtin.sy.get().index.set(unique664 + 1);
        ...
        const unique665 = abap.IntegerFactory.get(8).get();
    ...
    const unique667 = len.get();
    for (let unique668 = 0; unique668 < unique667; unique668++) {
      abap.builtin.sy.get().index.set(unique668 + 1);
