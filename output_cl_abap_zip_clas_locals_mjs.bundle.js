...
class lcl_stream {
  ...
  async append_crc(INPUT) {
    ...
    if (abap.compare.eq(abap.builtin.xstrlen({val: lcl_stream.crc32_map}), abap.IntegerFactory.get(0))) {
      ...
      const unique711 = new abap.types.Integer().set(256).get();
      for (let unique712 = 0; unique712 < unique711; unique712++) {
        abap.builtin.sy.get().index.set(unique712 + 1);
        ...
        const unique713 = abap.IntegerFactory.get(8).get();
        for (let unique714 = 0; unique714 < unique713; unique714++) {
          abap.builtin.sy.get().index.set(unique714 + 1);
    ...
    const unique715 = len.get();
    for (let unique716 = 0; unique716 < unique715; unique716++) {
      abap.builtin.sy.get().index.set(unique716 + 1);
