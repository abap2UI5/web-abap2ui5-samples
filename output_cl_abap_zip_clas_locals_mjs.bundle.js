...
class lcl_stream {
  ...
  async append_crc(INPUT) {
    ...
    if (abap.compare.eq(abap.builtin.xstrlen({val: lcl_stream.crc32_map}), abap.IntegerFactory.get(0))) {
      ...
      const unique662 = new abap.types.Integer().set(256).get();
      for (let unique663 = 0; unique663 < unique662; unique663++) {
        abap.builtin.sy.get().index.set(unique663 + 1);
        ...
        const unique664 = abap.IntegerFactory.get(8).get();
        for (let unique665 = 0; unique665 < unique664; unique665++) {
          abap.builtin.sy.get().index.set(unique665 + 1);
    ...
    const unique666 = len.get();
    for (let unique667 = 0; unique667 < unique666; unique667++) {
      abap.builtin.sy.get().index.set(unique667 + 1);
