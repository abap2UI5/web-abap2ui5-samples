...
class lcl_stream {
  ...
  async append_crc(INPUT) {
    ...
    if (abap.compare.eq(abap.builtin.xstrlen({val: lcl_stream.crc32_map}), abap.IntegerFactory.get(0))) {
      ...
      const unique665 = new abap.types.Integer().set(256).get();
      for (let unique666 = 0; unique666 < unique665; unique666++) {
        ...
        const unique667 = abap.IntegerFactory.get(8).get();
    ...
    const unique669 = len.get();
    for (let unique670 = 0; unique670 < unique669; unique670++) {
      abap.builtin.sy.get().index.set(unique670 + 1);
