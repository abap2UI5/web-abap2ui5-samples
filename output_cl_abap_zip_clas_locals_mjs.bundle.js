...
class lcl_stream {
  ...
  async append_crc(INPUT) {
    ...
    if (abap.compare.eq(abap.builtin.xstrlen({val: lcl_stream.crc32_map}), abap.IntegerFactory.get(0))) {
      ...
      const unique619 = new abap.types.Integer().set(256).get();
      for (let unique620 = 0; unique620 < unique619; unique620++) {
        ...
        const unique621 = abap.IntegerFactory.get(8).get();
    ...
    const unique623 = len.get();
    for (let unique624 = 0; unique624 < unique623; unique624++) {
      abap.builtin.sy.get().index.set(unique624 + 1);
