...
class lcl_stream {
  ...
  async append_crc(INPUT) {
    ...
    if (abap.compare.eq(abap.builtin.xstrlen({val: lcl_stream.crc32_map}), abap.IntegerFactory.get(0))) {
      ...
      const unique673 = new abap.types.Integer().set(256).get();
      for (let unique674 = 0; unique674 < unique673; unique674++) {
        abap.builtin.sy.get().index.set(unique674 + 1);
        ...
        const unique675 = abap.IntegerFactory.get(8).get();
        for (let unique676 = 0; unique676 < unique675; unique676++) {
          abap.builtin.sy.get().index.set(unique676 + 1);
    ...
    const unique677 = len.get();
    for (let unique678 = 0; unique678 < unique677; unique678++) {
      abap.builtin.sy.get().index.set(unique678 + 1);
