...
class lcl_stream {
  ...
  async append_crc(INPUT) {
    ...
    if (abap.compare.eq(abap.builtin.xstrlen({val: lcl_stream.crc32_map}), abap.IntegerFactory.get(0))) {
      ...
      const unique720 = new abap.types.Integer().set(256).get();
      for (let unique721 = 0; unique721 < unique720; unique721++) {
        abap.builtin.sy.get().index.set(unique721 + 1);
        ...
        const unique722 = abap.IntegerFactory.get(8).get();
        for (let unique723 = 0; unique723 < unique722; unique723++) {
          abap.builtin.sy.get().index.set(unique723 + 1);
    ...
    const unique724 = len.get();
    for (let unique725 = 0; unique725 < unique724; unique725++) {
      abap.builtin.sy.get().index.set(unique725 + 1);
