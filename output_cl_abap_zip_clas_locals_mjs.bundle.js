...
class lcl_stream {
  ...
  async append_crc(INPUT) {
    ...
    if (abap.compare.eq(abap.builtin.xstrlen({val: lcl_stream.crc32_map}), abap.IntegerFactory.get(0))) {
      ...
      const unique701 = new abap.types.Integer().set(256).get();
      for (let unique702 = 0; unique702 < unique701; unique702++) {
        abap.builtin.sy.get().index.set(unique702 + 1);
        ...
        const unique703 = abap.IntegerFactory.get(8).get();
        for (let unique704 = 0; unique704 < unique703; unique704++) {
          abap.builtin.sy.get().index.set(unique704 + 1);
    ...
    const unique705 = len.get();
    for (let unique706 = 0; unique706 < unique705; unique706++) {
      abap.builtin.sy.get().index.set(unique706 + 1);
