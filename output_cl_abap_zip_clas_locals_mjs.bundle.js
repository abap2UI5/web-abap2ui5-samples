...
class lcl_stream {
  ...
  async append_crc(INPUT) {
    ...
    if (abap.compare.eq(abap.builtin.xstrlen({val: lcl_stream.crc32_map}), abap.IntegerFactory.get(0))) {
      ...
      const unique759 = new abap.types.Integer().set(256).get();
      for (let unique760 = 0; unique760 < unique759; unique760++) {
        abap.builtin.sy.get().index.set(unique760 + 1);
        ...
        const unique761 = abap.IntegerFactory.get(8).get();
        for (let unique762 = 0; unique762 < unique761; unique762++) {
          abap.builtin.sy.get().index.set(unique762 + 1);
    ...
    const unique763 = len.get();
    for (let unique764 = 0; unique764 < unique763; unique764++) {
      abap.builtin.sy.get().index.set(unique764 + 1);
