...
class lcl_stream {
  ...
  async append_crc(INPUT) {
    ...
    if (abap.compare.eq(abap.builtin.xstrlen({val: lcl_stream.crc32_map}), abap.IntegerFactory.get(0))) {
      ...
      const unique742 = new abap.types.Integer().set(256).get();
      for (let unique743 = 0; unique743 < unique742; unique743++) {
        abap.builtin.sy.get().index.set(unique743 + 1);
        ...
        const unique744 = abap.IntegerFactory.get(8).get();
        for (let unique745 = 0; unique745 < unique744; unique745++) {
          abap.builtin.sy.get().index.set(unique745 + 1);
    ...
    const unique746 = len.get();
    for (let unique747 = 0; unique747 < unique746; unique747++) {
      abap.builtin.sy.get().index.set(unique747 + 1);
