...
class lcl_stream {
  ...
  async append_crc(INPUT) {
    ...
    if (abap.compare.eq(abap.builtin.xstrlen({val: lcl_stream.crc32_map}), abap.IntegerFactory.get(0))) {
      ...
      const unique698 = new abap.types.Integer().set(256).get();
      for (let unique699 = 0; unique699 < unique698; unique699++) {
        abap.builtin.sy.get().index.set(unique699 + 1);
        ...
        const unique700 = abap.IntegerFactory.get(8).get();
        for (let unique701 = 0; unique701 < unique700; unique701++) {
          abap.builtin.sy.get().index.set(unique701 + 1);
    ...
    const unique702 = len.get();
    for (let unique703 = 0; unique703 < unique702; unique703++) {
      abap.builtin.sy.get().index.set(unique703 + 1);
