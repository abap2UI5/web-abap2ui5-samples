...
class lcl_stream {
  ...
  async append_crc(INPUT) {
    ...
    if (abap.compare.eq(abap.builtin.xstrlen({val: lcl_stream.crc32_map}), abap.IntegerFactory.get(0))) {
      ...
      const unique738 = new abap.types.Integer().set(256).get();
      for (let unique739 = 0; unique739 < unique738; unique739++) {
        abap.builtin.sy.get().index.set(unique739 + 1);
        ...
        const unique740 = abap.IntegerFactory.get(8).get();
        for (let unique741 = 0; unique741 < unique740; unique741++) {
          abap.builtin.sy.get().index.set(unique741 + 1);
