...
class lcl_stream {
  ...
  async append_crc(INPUT) {
    ...
    if (abap.compare.eq(abap.builtin.xstrlen({val: lcl_stream.crc32_map}), abap.IntegerFactory.get(0))) {
      ...
      const unique697 = new abap.types.Integer().set(256).get();
      for (let unique698 = 0; unique698 < unique697; unique698++) {
        abap.builtin.sy.get().index.set(unique698 + 1);
        ...
        const unique699 = abap.IntegerFactory.get(8).get();
        for (let unique700 = 0; unique700 < unique699; unique700++) {
          abap.builtin.sy.get().index.set(unique700 + 1);
