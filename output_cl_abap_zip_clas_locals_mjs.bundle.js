...
class lcl_stream {
  ...
  async append_crc(INPUT) {
    ...
    if (abap.compare.eq(abap.builtin.xstrlen({val: lcl_stream.crc32_map}), abap.IntegerFactory.get(0))) {
      ...
      const unique700 = new abap.types.Integer().set(256).get();
      for (let unique701 = 0; unique701 < unique700; unique701++) {
        ...
        const unique702 = abap.IntegerFactory.get(8).get();
    ...
    const unique704 = len.get();
    for (let unique705 = 0; unique705 < unique704; unique705++) {
      abap.builtin.sy.get().index.set(unique705 + 1);
