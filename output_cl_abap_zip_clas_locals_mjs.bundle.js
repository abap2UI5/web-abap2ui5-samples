...
class lcl_stream {
  ...
  async append_crc(INPUT) {
    ...
    if (abap.compare.eq(abap.builtin.xstrlen({val: lcl_stream.crc32_map}), abap.IntegerFactory.get(0))) {
      ...
      const unique773 = new abap.types.Integer().set(256).get();
      for (let unique774 = 0; unique774 < unique773; unique774++) {
        abap.builtin.sy.get().index.set(unique774 + 1);
        ...
        const unique775 = abap.IntegerFactory.get(8).get();
        for (let unique776 = 0; unique776 < unique775; unique776++) {
          abap.builtin.sy.get().index.set(unique776 + 1);
    ...
    const unique777 = len.get();
    for (let unique778 = 0; unique778 < unique777; unique778++) {
      abap.builtin.sy.get().index.set(unique778 + 1);
