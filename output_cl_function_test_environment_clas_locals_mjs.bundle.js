...
class lcl_input_arguments {
  ...
  async if_ftd_input_arguments$get_importing_parameter(INPUT) {
    ...
    if (abap.compare.ne(abap.builtin.sy.get().subrc, abap.IntegerFactory.get(0))) {
      const unique926 = await (new abap.Classes['CX_FTD_PARAMETER_NOT_FOUND']()).constructor_();
      unique926.EXTRA_CX = {"INTERNAL_FILENAME": "cl_function_test_environment.clas.locals_imp.abap","INTERNAL_LINE": 19};
      throw unique926;
    }
  ...
  async if_ftd_input_arguments$get_table_parameter(INPUT) {
    ...
    abap.statements.readTable(this.mt_tables,{into: ls_row,
      ...
      withKeySimple: {"name": name}});
    if (abap.compare.ne(abap.builtin.sy.get().subrc, abap.IntegerFactory.get(0))) {
      const unique927 = await (new abap.Classes['CX_FTD_PARAMETER_NOT_FOUND']()).constructor_();
      unique927.EXTRA_CX = {"INTERNAL_FILENAME": "cl_function_test_environment.clas.locals_imp.abap","INTERNAL_LINE": 28};
      throw unique927;
    }
    result.set(ls_row.get().value);
    return result;
...
class lcl_invoker {
  ...
  static async invoke(INPUT) {
    ...
    for await (const unique928 of abap.statements.loop(lo_result.get().mt_exporting)) {
      ls_exporting.set(unique928);
      fminput.importing[ls_exporting.get().name.get().toLowerCase().trimEnd()].set(ls_exporting.get().value.dereference());
    }
    for await (const unique929 of abap.statements.loop(lo_result.get().mt_tables)) {
      ls_table.set(unique929);
