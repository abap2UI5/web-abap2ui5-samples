...
class lcl_dump {
  ...
  static async to_string(INPUT) {
    ...
    if (abap.compare.eq(unique906, abap.CharacterFactory.get(1, 'u')) || abap.compare.eq(unique906, abap.CharacterFactory.get(1, 'v'))) {
      rv_str.set((await this.dump_structure({iv_val: iv_val})));
    } else if (abap.compare.eq(unique906, abap.CharacterFactory.get(1, 'h'))) {
      rv_str.set(new abap.types.String().set(`[itab]`));
    } else if (abap.compare.eq(unique906, abap.CharacterFactory.get(1, 'r'))) {
  ...
  static async dump_structure(INPUT) {
    ...
    for await (const unique907 of abap.statements.loop(lt_components)) {
      lv_name.set(unique907);
