...
class lcl_json_parser {
  ...
  async parse(INPUT) {
    ...
    if (abap.compare.eq(lv_error, abap.builtin.abap_true)) {
      ...
      const unique854 = await (new abap.Classes['CX_SXML_PARSE_ERROR']()).constructor_({xml_offset: lv_xml_offset});
      unique854.EXTRA_CX = {"INTERNAL_FILENAME": "cl_sxml_string_reader.clas.locals_imp.abap","INTERNAL_LINE": 64};
      throw unique854;
    }
  ...
  async #traverse(INPUT) {
    ...
    let unique855 = lv_type;
    if (abap.compare.eq(unique855, abap.CharacterFactory.get(6, 'object'))) {
      await this.#traverse_object({iv_json: iv_json, iv_key: iv_key});
    } else if (abap.compare.eq(unique855, abap.CharacterFactory.get(5, 'array'))) {
      await this.#traverse_array({iv_json: iv_json, iv_key: iv_key});
    } else if (abap.compare.eq(unique855, abap.CharacterFactory.get(6, 'string')) || abap.compare.eq(unique855, abap.CharacterFactory.get(7, 'boolean')) || abap.compare.eq(unique855, abap.CharacterFactory.get(6, 'number')) || abap.compare.eq(unique855, abap.CharacterFactory.get(4, 'null'))) {
      iv_json = iv_json.value + "";
      let unique856 = lv_type;
      if (abap.compare.eq(unique856, abap.CharacterFactory.get(6, 'string'))) {
        lv_type.set(abap.CharacterFactory.get(3, 'str'));
      } else if (abap.compare.eq(unique856, abap.CharacterFactory.get(6, 'number'))) {
        lv_type.set(abap.CharacterFactory.get(3, 'num'));
      } else if (abap.compare.eq(unique856, abap.CharacterFactory.get(7, 'boolean'))) {
  ...
  async #traverse_array(INPUT) {
    ...
    const unique857 = lv_length.get();
    for (let unique858 = 0; unique858 < unique857; unique858++) {
      abap.builtin.sy.get().index.set(unique858 + 1);
...
class lcl_reader {
  ...
  async #initialize() {
    ...
    lo_json.clear();
    for await (const unique859 of abap.statements.loop(lt_parsed.dereference())) {
      fs_ls_parsed_.assign(unique859);
      let unique860 = fs_ls_parsed_.get().type;
      if (abap.compare.eq(unique860, abap.Classes['IF_SXML_NODE'].if_sxml_node$co_nt_element_open)) {
        lt_attributes.clear();
      ...
      } else if (abap.compare.eq(unique860, abap.Classes['IF_SXML_NODE'].if_sxml_node$co_nt_element_close)) {
        li_node.set(await (new abap.Classes['CLAS-CL_SXML_STRING_READER-LCL_CLOSE_NODE']()).constructor_({name: fs_ls_parsed_.get().name}));
      } else if (abap.compare.eq(unique860, abap.Classes['IF_SXML_NODE'].if_sxml_node$co_nt_value)) {
  ...
  async if_sxml_reader$read_next_node() {
    ...
    if (abap.compare.initial(node) === false) {
      ...
      let unique861 = this.if_sxml_reader$node_type;
      if (abap.compare.eq(unique861, abap.Classes['IF_SXML_NODE'].if_sxml_node$co_nt_element_open)) {
      ...
      } else if (abap.compare.eq(unique861, abap.Classes['IF_SXML_NODE'].if_sxml_node$co_nt_element_close)) {
      ...
      } else if (abap.compare.eq(unique861, abap.Classes['IF_SXML_NODE'].if_sxml_node$co_nt_value)) {
