...
class lcl_named_node_map {
  ...
  async if_ixml_named_node_map$get_named_item_ns(INPUT) {
    ...
    for await (const unique704 of abap.statements.loop(this.mt_list)) {
      li_node.set(unique704);
...
class lcl_node {
  ...
  async if_ixml_element$find_from_name_ns(INPUT) {
    ...
    for await (const unique705 of abap.statements.loop(lt_nodes)) {
      li_top.set(unique705);
...
class lcl_parser {
  ...
  async if_ixml_parser$parse() {
    ...
    while (abap.compare.initial(lv_xml) === false) {
      abap.builtin.sy.get().index.set(unique711++);
  ...
  async parse_attributes(INPUT) {
    ...
    let unique712 = 1;
    while (true) {
      abap.builtin.sy.get().index.set(unique712++);
