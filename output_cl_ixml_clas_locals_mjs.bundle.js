...
class lcl_named_node_map {
  ...
  async if_ixml_named_node_map$get_named_item_ns(INPUT) {
    ...
    for await (const unique757 of abap.statements.loop(this.mt_list)) {
      li_node.set(unique757);
...
class lcl_node {
  ...
  async if_ixml_element$find_from_name_ns(INPUT) {
    ...
    for await (const unique758 of abap.statements.loop(lt_nodes)) {
      li_top.set(unique758);
      ...
      let unique759 = 1;
      while (true) {
        abap.builtin.sy.get().index.set(unique759++);
  ...
  async if_ixml_element$render(INPUT) {
    ...
    let unique760 = 1;
    while (true) {
      abap.builtin.sy.get().index.set(unique760++);
      li_node.set((await li_iterator.get().if_ixml_node_iterator$get_next()));
      if (abap.compare.initial(li_node)) {
        break;
      }
    ...
    let unique761 = 1;
    while (true) {
      abap.builtin.sy.get().index.set(unique761++);
  ...
  async if_ixml_node$get_depth() {
    ...
    if (abap.compare.eq((await this.mo_children.get().if_ixml_node_list$get_length()), abap.IntegerFactory.get(0))) {
      ...
      let unique762 = 1;
      while (true) {
        abap.builtin.sy.get().index.set(unique762++);
        li_node.set((await li_iterator.get().if_ixml_node_iterator$get_next()));
        if (abap.compare.initial(li_node)) {
          break;
        }
  ...
  async if_ixml_node$get_value() {
    ...
    if (abap.compare.eq((await this.mo_children.get().if_ixml_node_list$get_length()), abap.IntegerFactory.get(0))) {
      ...
      let unique763 = 1;
      while (true) {
        abap.builtin.sy.get().index.set(unique763++);
        li_node.set((await li_iterator.get().if_ixml_node_iterator$get_next()));
        if (abap.compare.initial(li_node)) {
          break;
        }
...
class lcl_parser {
  ...
  async if_ixml_parser$parse() {
    ...
    while (abap.compare.initial(lv_xml) === false) {
      abap.builtin.sy.get().index.set(unique764++);
      lo_node.clear();
  ...
  async #parse_attributes(INPUT) {
    ...
    let unique765 = 1;
    while (true) {
      abap.builtin.sy.get().index.set(unique765++);
