"use strict";
(self["webpackChunkabap2ui5_web"] = self["webpackChunkabap2ui5_web"] || []).push([["output_cl_ixml_clas_locals_mjs"],{
...
class lcl_named_node_map {
  ...
  async if_ixml_named_node_map$get_named_item_ns(INPUT) {
    ...
    for await (const unique658 of abap.statements.loop(this.mt_list)) {
      li_node.set(unique658);
...
class lcl_node {
  ...
  async if_ixml_element$find_from_name_ns(INPUT) {
    ...
    for await (const unique659 of abap.statements.loop(lt_nodes)) {
      li_top.set(unique659);
      ...
      let unique660 = 1;
      while (true) {
        abap.builtin.sy.get().index.set(unique660++);
  ...
  async if_ixml_element$render(INPUT) {
    ...
    let unique661 = 1;
    while (true) {
      abap.builtin.sy.get().index.set(unique661++);
      li_node.set((await li_iterator.get().if_ixml_node_iterator$get_next()));
      if (abap.compare.initial(li_node)) {
        break;
      }
    ...
    let unique662 = 1;
    while (true) {
      abap.builtin.sy.get().index.set(unique662++);
  ...
  async if_ixml_node$get_depth() {
    ...
    if (abap.compare.eq((await this.mo_children.get().if_ixml_node_list$get_length()), abap.IntegerFactory.get(0))) {
      ...
      let unique663 = 1;
      while (true) {
        abap.builtin.sy.get().index.set(unique663++);
        li_node.set((await li_iterator.get().if_ixml_node_iterator$get_next()));
        if (abap.compare.initial(li_node)) {
          break;
        }
  ...
  async if_ixml_node$get_value() {
    ...
    if (abap.compare.eq((await this.mo_children.get().if_ixml_node_list$get_length()), abap.IntegerFactory.get(0))) {
      ...
      let unique664 = 1;
      while (true) {
        abap.builtin.sy.get().index.set(unique664++);
        li_node.set((await li_iterator.get().if_ixml_node_iterator$get_next()));
        if (abap.compare.initial(li_node)) {
          break;
        }
...
class lcl_parser {
  ...
  async if_ixml_parser$parse() {
    ...
    while (abap.compare.initial(lv_xml) === false) {
      abap.builtin.sy.get().index.set(unique665++);
  ...
  async parse_attributes(INPUT) {
    ...
    let unique666 = 1;
    while (true) {
      abap.builtin.sy.get().index.set(unique666++);
