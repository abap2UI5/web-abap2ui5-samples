...
class lcl_named_node_map {
  ...
  async if_ixml_named_node_map$get_named_item_ns(INPUT) {
    ...
    for await (const unique701 of abap.statements.loop(this.mt_list)) {
      li_node.set(unique701);
...
class lcl_node {
  ...
  async if_ixml_element$find_from_name_ns(INPUT) {
    ...
    for await (const unique702 of abap.statements.loop(lt_nodes)) {
      li_top.set(unique702);
      ...
      let unique703 = 1;
      while (true) {
        abap.builtin.sy.get().index.set(unique703++);
  ...
  async if_ixml_element$render(INPUT) {
    ...
    let unique704 = 1;
    while (true) {
      abap.builtin.sy.get().index.set(unique704++);
      li_node.set((await li_iterator.get().if_ixml_node_iterator$get_next()));
      if (abap.compare.initial(li_node)) {
        break;
      }
    ...
    let unique705 = 1;
    while (true) {
      abap.builtin.sy.get().index.set(unique705++);
  ...
  async if_ixml_node$get_depth() {
    ...
    if (abap.compare.eq((await this.mo_children.get().if_ixml_node_list$get_length()), abap.IntegerFactory.get(0))) {
      ...
      let unique706 = 1;
      while (true) {
        abap.builtin.sy.get().index.set(unique706++);
        li_node.set((await li_iterator.get().if_ixml_node_iterator$get_next()));
        if (abap.compare.initial(li_node)) {
          break;
        }
  ...
  async if_ixml_node$get_value() {
    ...
    if (abap.compare.eq((await this.mo_children.get().if_ixml_node_list$get_length()), abap.IntegerFactory.get(0))) {
      ...
      let unique707 = 1;
      while (true) {
        abap.builtin.sy.get().index.set(unique707++);
        li_node.set((await li_iterator.get().if_ixml_node_iterator$get_next()));
        if (abap.compare.initial(li_node)) {
          break;
        }
...
class lcl_parser {
  ...
  async if_ixml_parser$parse() {
    ...
    while (abap.compare.initial(lv_xml) === false) {
      abap.builtin.sy.get().index.set(unique708++);
  ...
  async parse_attributes(INPUT) {
    ...
    let unique709 = 1;
    while (true) {
      abap.builtin.sy.get().index.set(unique709++);
