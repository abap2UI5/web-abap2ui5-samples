...
class lcl_named_node_map {
  ...
  async if_ixml_named_node_map$get_named_item_ns(INPUT) {
    ...
    for await (const unique781 of abap.statements.loop(this.mt_list)) {
      li_node.set(unique781);
...
class lcl_node {
  ...
  async if_ixml_element$find_from_name_ns(INPUT) {
    ...
    for await (const unique782 of abap.statements.loop(lt_nodes)) {
      li_top.set(unique782);
  ...
  async if_ixml_element$render(INPUT) {
    ...
    let unique784 = 1;
    while (true) {
      abap.builtin.sy.get().index.set(unique784++);
      li_node.set((await li_iterator.get().if_ixml_node_iterator$get_next()));
      if (abap.compare.initial(li_node)) {
        break;
      }
    ...
    let unique785 = 1;
    while (true) {
      abap.builtin.sy.get().index.set(unique785++);
  ...
  async if_ixml_node$get_depth() {
    ...
    if (abap.compare.eq((await this.mo_children.get().if_ixml_node_list$get_length()), abap.IntegerFactory.get(0))) {
      ...
      let unique786 = 1;
      while (true) {
        abap.builtin.sy.get().index.set(unique786++);
        li_node.set((await li_iterator.get().if_ixml_node_iterator$get_next()));
        if (abap.compare.initial(li_node)) {
          break;
        }
  ...
  async if_ixml_node$get_value() {
    ...
    if (abap.compare.eq((await this.mo_children.get().if_ixml_node_list$get_length()), abap.IntegerFactory.get(0))) {
      ...
      let unique787 = 1;
      while (true) {
        abap.builtin.sy.get().index.set(unique787++);
        li_node.set((await li_iterator.get().if_ixml_node_iterator$get_next()));
        if (abap.compare.initial(li_node)) {
          break;
        }
...
class lcl_parser {
  ...
  async if_ixml_parser$parse() {
    ...
    while (abap.compare.initial(lv_xml) === false) {
      abap.builtin.sy.get().index.set(unique788++);
  ...
  async parse_attributes(INPUT) {
    ...
    let unique789 = 1;
    while (true) {
      abap.builtin.sy.get().index.set(unique789++);
