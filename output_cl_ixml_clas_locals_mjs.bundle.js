...
class lcl_named_node_map {
  ...
  async if_ixml_named_node_map$get_named_item_ns(INPUT) {
    ...
    for await (const unique775 of abap.statements.loop(this.mt_list)) {
      li_node.set(unique775);
...
class lcl_node {
  ...
  async if_ixml_element$find_from_name_ns(INPUT) {
    ...
    for await (const unique776 of abap.statements.loop(lt_nodes)) {
      li_top.set(unique776);
      ...
      let unique777 = 1;
      while (true) {
        abap.builtin.sy.get().index.set(unique777++);
  ...
  async if_ixml_element$render(INPUT) {
    ...
    let unique778 = 1;
    while (true) {
      abap.builtin.sy.get().index.set(unique778++);
      li_node.set((await li_iterator.get().if_ixml_node_iterator$get_next()));
      if (abap.compare.initial(li_node)) {
        break;
      }
    ...
    let unique779 = 1;
    while (true) {
      abap.builtin.sy.get().index.set(unique779++);
  ...
  async if_ixml_node$get_depth() {
    ...
    if (abap.compare.eq((await this.mo_children.get().if_ixml_node_list$get_length()), abap.IntegerFactory.get(0))) {
      ...
      let unique780 = 1;
      while (true) {
        abap.builtin.sy.get().index.set(unique780++);
        li_node.set((await li_iterator.get().if_ixml_node_iterator$get_next()));
        if (abap.compare.initial(li_node)) {
          break;
        }
  ...
  async if_ixml_node$get_value() {
    ...
    if (abap.compare.eq((await this.mo_children.get().if_ixml_node_list$get_length()), abap.IntegerFactory.get(0))) {
      ...
      let unique781 = 1;
      while (true) {
        abap.builtin.sy.get().index.set(unique781++);
        li_node.set((await li_iterator.get().if_ixml_node_iterator$get_next()));
        if (abap.compare.initial(li_node)) {
          break;
        }
...
class lcl_parser {
  ...
  async if_ixml_parser$parse() {
    ...
    while (abap.compare.initial(lv_xml) === false) {
      abap.builtin.sy.get().index.set(unique782++);
      lo_node.clear();
  ...
  async #parse_attributes(INPUT) {
    ...
    let unique783 = 1;
    while (true) {
      abap.builtin.sy.get().index.set(unique783++);
