"use strict";
(self["webpackChunkabap2ui5"] = self["webpackChunkabap2ui5"] || []).push([["output_cl_ixml_clas_locals_mjs"],{
...
class lcl_named_node_map {
  ...
  async if_ixml_named_node_map$get_named_item_ns(INPUT) {
    ...
    for await (const unique798 of abap.statements.loop(this.mt_list)) {
      li_node.set(unique798);
...
class lcl_node {
  ...
  async if_ixml_node$num_children() {
    let rval = new abap.types.Integer({qualifiedName: "I"});
    abap.statements.assert(abap.compare.eq(abap.IntegerFactory.get(1), abap.CharacterFactory.get(4, 'todo')));
    return rval;
  }
  ...
  async if_ixml_element$find_from_name_ns(INPUT) {
    ...
    for await (const unique799 of abap.statements.loop(lt_nodes)) {
      li_top.set(unique799);
      ...
      let unique800 = 1;
      while (true) {
        abap.builtin.sy.get().index.set(unique800++);
  ...
  async #has_direct_text() {
    ...
    if (abap.compare.eq((await li_child.get().if_ixml_node$get_name()), abap.CharacterFactory.get(5, '#text'))) {
  ...
  async if_ixml_element$render(INPUT) {
    ...
    let unique801 = 1;
    while (true) {
      abap.builtin.sy.get().index.set(unique801++);
      li_node.set((await li_iterator.get().if_ixml_node_iterator$get_next()));
      if (abap.compare.initial(li_node)) {
        break;
      }
    ...
    li_children.set((await this.if_ixml_node$get_children()));
    if (abap.compare.ne(this.mv_name, abap.CharacterFactory.get(5, '#text')) && abap.compare.eq((await ostream.get().if_ixml_ostream$get_pretty_print()), abap.builtin.abap_true)) {
    ...
    let unique802 = 1;
    while (true) {
      abap.builtin.sy.get().index.set(unique802++);
    ...
    if (abap.compare.eq((await ostream.get().if_ixml_ostream$get_pretty_print()), abap.builtin.abap_true) && abap.compare.ne(this.mv_name, abap.CharacterFactory.get(5, '#text'))) {
      await ostream.get().if_ixml_ostream$write_string({string: new abap.types.String().set(`\n`)});
    }
  }
  ...
  async if_ixml_node$get_depth() {
    ...
    if (abap.compare.eq((await this.mo_children.get().if_ixml_node_list$get_length()), abap.IntegerFactory.get(0))) {
      ...
      let unique803 = 1;
      while (true) {
        abap.builtin.sy.get().index.set(unique803++);
        li_node.set((await li_iterator.get().if_ixml_node_iterator$get_next()));
        if (abap.compare.initial(li_node)) {
          break;
        }
  ...
  async if_ixml_node$get_value() {
    ...
    if (abap.compare.eq((await this.mo_children.get().if_ixml_node_list$get_length()), abap.IntegerFactory.get(0))) {
      ...
      let unique804 = 1;
      while (true) {
        abap.builtin.sy.get().index.set(unique804++);
        li_node.set((await li_iterator.get().if_ixml_node_iterator$get_next()));
        if (abap.compare.initial(li_node)) {
          break;
        }
...
class lcl_document {
  ...
  async if_ixml_document$find_from_name_ns(INPUT) {
    ...
    if (name?.getQualifiedName === undefined || name.getQualifiedName() !== "STRING") { name = undefined; }
    if (name === undefined) { name = new abap.types.String({qualifiedName: "STRING"}).set(INPUT.name); }
    element.set((await this.mi_node.get().if_ixml_element$find_from_name_ns({name: name, depth: depth, namespace: abap.CharacterFactory.get(1, '')})));
    return element;
  }
...
class lcl_parser {
  ...
  static METHODS = {"PARSE_ATTRIBUTES": {"visibility": "I", "parameters": {"II_NODE": {"type": () => {return new abap.types.ABAPObject({qualifiedName: "IF_IXML_NODE", RTTIName: "\\INTERFACE=IF_IXML_NODE"});}, "is_optional": " "}, "IV_XML": {"type": () => {return new abap.types.String({qualifiedName: "STRING"});}, "is_optional": " "}, "IS_MATCH": {"type": () => {return new abap.types.Structure({
  ...
  "offset": new abap.types.Integer({qualifiedName: "I"}),
  "length": new abap.types.Integer({qualifiedName: "I"})}, "SUBMATCH_RESULT", "SUBMATCH_RESULT", {}, {}), {"withHeader":false,"keyType":"DEFAULT","primaryKey":{"isUnique":false,"type":"STANDARD","keyFields":[],"name":"primary_key"},"secondary":[]}, "SUBMATCH_RESULT_TAB")}, "MATCH_RESULT", "MATCH_RESULT", {}, {});}, "is_optional": " "}}},
  ...
  async if_ixml_parser$parse() {
    ...
    while (abap.compare.initial(lv_xml) === false) {
      abap.builtin.sy.get().index.set(unique805++);
      lo_node.clear();
      if (abap.compare.cp(lv_xml, abap.CharacterFactory.get(7, '<?xml *'))) {
  ...
  async #parse_attributes(INPUT) {
    ...
    let unique806 = 1;
    while (true) {
      abap.builtin.sy.get().index.set(unique806++);
