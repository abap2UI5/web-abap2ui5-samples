...
class lcl_named_node_map {
  ...
  async if_ixml_named_node_map$get_named_item_ns(INPUT) {
    ...
    for await (const unique750 of abap.statements.loop(this.mt_list)) {
      li_node.set(unique750);
...
class lcl_node {
  ...
  async if_ixml_element$find_from_name_ns(INPUT) {
    ...
    for await (const unique751 of abap.statements.loop(lt_nodes)) {
      li_top.set(unique751);
      ...
      let unique752 = 1;
      while (true) {
        abap.builtin.sy.get().index.set(unique752++);
  ...
  async if_ixml_element$render(INPUT) {
    ...
    let unique753 = 1;
    while (true) {
      abap.builtin.sy.get().index.set(unique753++);
      li_node.set((await li_iterator.get().if_ixml_node_iterator$get_next()));
      if (abap.compare.initial(li_node)) {
        break;
      }
    ...
    let unique754 = 1;
    while (true) {
      abap.builtin.sy.get().index.set(unique754++);
  ...
  async if_ixml_node$get_depth() {
    ...
    if (abap.compare.eq((await this.mo_children.get().if_ixml_node_list$get_length()), abap.IntegerFactory.get(0))) {
      ...
      let unique755 = 1;
      while (true) {
        abap.builtin.sy.get().index.set(unique755++);
        li_node.set((await li_iterator.get().if_ixml_node_iterator$get_next()));
        if (abap.compare.initial(li_node)) {
          break;
        }
  ...
  async if_ixml_node$get_value() {
    ...
    if (abap.compare.eq((await this.mo_children.get().if_ixml_node_list$get_length()), abap.IntegerFactory.get(0))) {
      ...
      let unique756 = 1;
      while (true) {
        abap.builtin.sy.get().index.set(unique756++);
        li_node.set((await li_iterator.get().if_ixml_node_iterator$get_next()));
        if (abap.compare.initial(li_node)) {
          break;
        }
...
class lcl_parser {
  ...
  async if_ixml_parser$parse() {
    ...
    while (abap.compare.initial(lv_xml) === false) {
      abap.builtin.sy.get().index.set(unique757++);
  ...
  async parse_attributes(INPUT) {
    ...
    let unique758 = 1;
    while (true) {
      abap.builtin.sy.get().index.set(unique758++);
