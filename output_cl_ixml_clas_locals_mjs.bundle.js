...
class lcl_named_node_map {
  ...
  async if_ixml_named_node_map$get_named_item_ns(INPUT) {
    ...
    for await (const unique712 of abap.statements.loop(this.mt_list)) {
      li_node.set(unique712);
...
class lcl_node {
  ...
  async if_ixml_element$find_from_name_ns(INPUT) {
    ...
    for await (const unique713 of abap.statements.loop(lt_nodes)) {
      li_top.set(unique713);
      ...
      let unique714 = 1;
      while (true) {
        abap.builtin.sy.get().index.set(unique714++);
  ...
  async if_ixml_element$render(INPUT) {
    ...
    let unique715 = 1;
    while (true) {
      abap.builtin.sy.get().index.set(unique715++);
      li_node.set((await li_iterator.get().if_ixml_node_iterator$get_next()));
      if (abap.compare.initial(li_node)) {
        break;
      }
    ...
    let unique716 = 1;
    while (true) {
      abap.builtin.sy.get().index.set(unique716++);
  ...
  async if_ixml_node$get_depth() {
    ...
    if (abap.compare.eq((await this.mo_children.get().if_ixml_node_list$get_length()), abap.IntegerFactory.get(0))) {
      ...
      let unique717 = 1;
      while (true) {
        abap.builtin.sy.get().index.set(unique717++);
        li_node.set((await li_iterator.get().if_ixml_node_iterator$get_next()));
        if (abap.compare.initial(li_node)) {
          break;
        }
  ...
  async if_ixml_node$get_value() {
    ...
    if (abap.compare.eq((await this.mo_children.get().if_ixml_node_list$get_length()), abap.IntegerFactory.get(0))) {
      ...
      let unique718 = 1;
      while (true) {
        abap.builtin.sy.get().index.set(unique718++);
        li_node.set((await li_iterator.get().if_ixml_node_iterator$get_next()));
        if (abap.compare.initial(li_node)) {
          break;
        }
...
class lcl_parser {
  ...
  async if_ixml_parser$parse() {
    ...
    while (abap.compare.initial(lv_xml) === false) {
      abap.builtin.sy.get().index.set(unique719++);
  ...
  async parse_attributes(INPUT) {
    ...
    let unique720 = 1;
    while (true) {
      abap.builtin.sy.get().index.set(unique720++);
