...
class lcl_named_node_map {
  ...
  async if_ixml_named_node_map$get_named_item_ns(INPUT) {
    ...
    for await (const unique812 of abap.statements.loop(this.mt_list)) {
      li_node.set(unique812);
...
class lcl_node {
  ...
  async if_ixml_element$find_from_name_ns(INPUT) {
    ...
    for await (const unique813 of abap.statements.loop(lt_nodes)) {
      li_top.set(unique813);
      ...
      let unique814 = 1;
      while (true) {
        abap.builtin.sy.get().index.set(unique814++);
  ...
  async if_ixml_element$render(INPUT) {
    ...
    let unique815 = 1;
    while (true) {
      abap.builtin.sy.get().index.set(unique815++);
      li_node.set((await li_iterator.get().if_ixml_node_iterator$get_next()));
      if (abap.compare.initial(li_node)) {
        break;
      }
    ...
    let unique816 = 1;
    while (true) {
      abap.builtin.sy.get().index.set(unique816++);
  ...
  async if_ixml_node$get_depth() {
    ...
    if (abap.compare.eq((await this.mo_children.get().if_ixml_node_list$get_length()), abap.IntegerFactory.get(0))) {
      ...
      let unique817 = 1;
      while (true) {
        abap.builtin.sy.get().index.set(unique817++);
        li_node.set((await li_iterator.get().if_ixml_node_iterator$get_next()));
        if (abap.compare.initial(li_node)) {
          break;
        }
  ...
  async if_ixml_node$get_value() {
    ...
    if (abap.compare.eq((await this.mo_children.get().if_ixml_node_list$get_length()), abap.IntegerFactory.get(0))) {
      ...
      let unique818 = 1;
      while (true) {
        abap.builtin.sy.get().index.set(unique818++);
        li_node.set((await li_iterator.get().if_ixml_node_iterator$get_next()));
        if (abap.compare.initial(li_node)) {
          break;
        }
...
class lcl_parser {
  ...
  async if_ixml_parser$parse() {
    ...
    while (abap.compare.initial(lv_xml) === false) {
      abap.builtin.sy.get().index.set(unique819++);
  ...
  async parse_attributes(INPUT) {
    ...
    let unique820 = 1;
    while (true) {
      abap.builtin.sy.get().index.set(unique820++);
