...
class lcl_heap {
  ...
  async add_object(INPUT) {
    ...
    lv_name.set(lo_descr.get().relative_name);
    for await (const unique760 of abap.statements.loop(lo_descr.get().interfaces)) {
      ls_interface.set(unique760);
    ...
    if (abap.compare.eq(is_serializable, abap.builtin.abap_true)) {
      ...
      for await (const unique761 of abap.statements.loop(lo_descr.get().attributes,{where: async (I) => {return abap.compare.eq(I.is_class, abap.builtin.abap_false);},topEquals: {"is_class": abap.builtin.abap_false}})) {
        ls_attribute.set(unique761);
...
class lcl_data_to_xml {
  ...
  async run(INPUT) {
    ...
    let unique762 = lo_type.get().kind;
    if (abap.compare.eq(unique762, abap.Classes['CL_ABAP_TYPEDESCR'].kind_struct)) {
      ...
      rv_xml.set(abap.operators.concat(rv_xml,new abap.types.String().set(`<${abap.templateFormatting(iv_name)}>`)));
      for await (const unique763 of abap.statements.loop(lt_comps)) {
        ls_compo.set(unique763);
      ...
      rv_xml.set(abap.operators.concat(rv_xml,new abap.types.String().set(`</${abap.templateFormatting(iv_name)}>`)));
    } else if (abap.compare.eq(unique762, abap.Classes['CL_ABAP_TYPEDESCR'].kind_elem)) {
    ...
    } else if (abap.compare.eq(unique762, abap.Classes['CL_ABAP_TYPEDESCR'].kind_table)) {
      ...
      rv_xml.set(abap.operators.concat(rv_xml,new abap.types.String().set(`<${abap.templateFormatting(iv_name)}>`)));
      for await (const unique764 of abap.statements.loop(fs_table_)) {
        fs_any_.assign(unique764);
        lv_ref.assign(fs_any_.getPointer());
        rv_xml.set(abap.operators.concat(rv_xml,(await this.run({iv_name: new abap.types.String().set(`item`), iv_ref: lv_ref}))));
      }
      rv_xml.set(abap.operators.concat(rv_xml,new abap.types.String().set(`</${abap.templateFormatting(iv_name)}>`)));
    } else if (abap.compare.eq(unique762, abap.Classes['CL_ABAP_TYPEDESCR'].kind_ref)) {
      let unique765 = lo_type.get().type_kind;
      if (abap.compare.eq(unique765, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_oref)) {
  ...
  static async run(INPUT) {
    ...
    for (const name in INPUT.source) {
      ...
      static async traverse_write_type(INPUT) {
        ...
        let lo_type = new abap.types.ABAPObject({qualifiedName: "CL_ABAP_TYPEDESCR", RTTIName: "\\CLASS=CL_ABAP_TYPEDESCR"});
        lo_type.set((await abap.Classes['CL_ABAP_TYPEDESCR'].describe_by_data({p_data: iv_ref.dereference()})));
        let unique766 = lo_type.get().type_kind;
        if (abap.compare.eq(unique766, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_int) || abap.compare.eq(unique766, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_int1) || abap.compare.eq(unique766, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_int2) || abap.compare.eq(unique766, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_int8) || abap.compare.eq(unique766, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_decfloat) || abap.compare.eq(unique766, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_decfloat16) || abap.compare.eq(unique766, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_decfloat34)) {
        ...
        if (INPUT.source.constructor.name === "Object") {
            ...
            class lcl_object_to_ixml {
              ...
              static async run(INPUT) {
                ...
                for await (const unique770 of abap.statements.loop(lt_stab)) {
                  ls_stab.set(unique770);
              ...
              static async traverse(INPUT) {
                ...
                let fs_field_ = new abap.types.FieldSymbol(new abap.types.Character(4));
                lo_type.set((await abap.Classes['CL_ABAP_TYPEDESCR'].describe_by_data({p_data: iv_ref.dereference()})));
                let unique771 = lo_type.get().kind;
                if (abap.compare.eq(unique771, abap.Classes['CL_ABAP_TYPEDESCR'].kind_struct)) {
                  await abap.statements.cast(lo_struc, lo_type);
                  lt_comps.set((await lo_struc.get().get_components()));
                  abap.statements.assign({target: fs_any_, source: iv_ref.dereference()});
                  for await (const unique772 of abap.statements.loop(lt_comps)) {
                    ls_compo.set(unique772);
                ...
                } else if (abap.compare.eq(unique771, abap.Classes['CL_ABAP_TYPEDESCR'].kind_elem)) {
                  await ii_parent.get().if_ixml_element$set_value({value: new abap.types.String().set(`${abap.templateFormatting(iv_ref.dereference())}`)});
                } else if (abap.compare.eq(unique771, abap.Classes['CL_ABAP_TYPEDESCR'].kind_table)) {
                  abap.statements.assign({target: fs_table_, source: iv_ref.dereference()});
                  for await (const unique773 of abap.statements.loop(fs_table_)) {
                    fs_any_.assign(unique773);
